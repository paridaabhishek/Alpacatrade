...
#         with open(trans_starter_file, "w") as transfile:
...
#         )
#         print(".. No transaction will happen though the signals will be generated !!!")

# else:
#     print("Not Enough tickers to start the trasaction!!!!")

#     # for key in dict(list(zip(stocks + cryptos, equity_pct))):
#     #     print(key)
#     #     print(dict(list(zip(stocks + cryptos, equity_pct)))[key])
...
def log_transaction(ticker):
    ...
    ].to_csv(tarns_path + r"/Crypto_" + ticker + "_tran_details.csv", index=None)
...
def trans_5Min_921EMA_stock():
    print("Trans for 5Min 921EMA started ")
    if path.exists(trans_starter_file):
        print(trans_starter_file)
        with open(trans_starter_file, "r") as transstart:
            asset_info = [line.rstrip() for line in transstart]
            print(asset_info[0])
...
# con = get_account_connection_api()
# print(con.get_account().account_blocked)
...
#     # print(conn.get_position(ticker))
#     print([s for s in conn.list_positions()])
#     return "r"


# r = crypto_buy("BTCUSD", 0.002)
# print(r.id)
# # messageJson = (
# #     str(r)
# #     .split("(")[1]
...
# # print(json.loads(messageJson))
# # print(dict(messageJson))
# # print(messageJson)
# # print(type(ast.literal_eval(messageJson)))
# di = ast.literal_eval(messageJson)
# sr = pd.Series(di)
# sr.to_csv("test.csv")

# df = pd.DataFrame(sr)
# print(df.transpose().to_csv("test.csv", index=None))
...
#     print("Selling the crypto with selling function  ")
#     conn = get_account_connection_api()
#     if ticker in [s for s in conn.list_positions()]:
#         c = conn.close_position(ticker)
#     # c = conn.list_orders(ticker)
#     else:
#         c = "No Position to sell"
#     return c


# c = crypto_sell("BTCUSD")
# print(c)


# print(if_positions_exists("AAPL"))
